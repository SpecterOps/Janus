...
description: Use this skill for Janus requests that ask what to measure, how to measure it, which analyzer should answer it, or how to implement or adjust source-aware analysis across Janus-supported C2 telemetry.
---
...
2. Determine the source: `mythic`, `ghostwriter`, `cobaltstrike`, or a cross-source comparison. If the source is omitted, infer it from the task or config only when low-risk.
...
4. Route by source and read the matching bundled reference before drilling into parser code:
   - Mythic: `references/mythic.md`, then `Parsers/Mythic/mythic_pull.py`
   - Partial Mythic: `references/mythic.md`, then `Parsers/Mythic/partial_data_adapter.py`
   - Ghostwriter: `references/ghostwriter.md`, then `Parsers/Ghostwriter/main.py`
   - Cobalt Strike: `references/cobaltstrike.md`, then `Parsers/CobaltStrike/cobalt_strike_rest.py`
   - Cross-source comparison: `docs/architecture.md`, then the relevant source references above
...
- Treat weaker source fidelity as a capability boundary, not as a reason to flatten stronger sources.
- Treat Ghostwriter result state as unreliable unless the source schema actually adds trustworthy status fields.
- If a metric depends on success/error truth, say explicitly which Janus-supported sources can support it today.
...
- Parser robustness guardrails now exist and should be preserved:
  ...
  - `run_merge()` remaps missing or duplicate `operation_id` values to deterministic unique IDs to avoid cross-operation key collisions.
...
- Which bundled source reference informed the source-specific guidance
